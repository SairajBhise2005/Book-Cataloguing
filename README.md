...
2. **Four ways to start:**
   ...
   - **Use Photo Instead** — single-book photo path; sends one cover to Gemini.
   - **Batch Photo Mode** — capture up to 5 or 10 covers (your choice) in a row and send them to Gemini in **one** request. Reviews each book sequentially. The biggest quota-stretcher available.
3. **Review** — verify title/author, pick Language / Genre / Reading Level from dropdowns, fill Location/Shelf, Status, Notes.
4. **Save to Google Sheet** — appends a row with an auto-incremented `#` serial number. In batch mode, save automatically advances to the next book; **Skip This Book** drops a bad result and moves on without saving.

### Batch mode tips
- The **Batch size** dropdown remembers your last choice (5 or 10) — set it once, stays put.
- Each captured photo appears as a thumbnail; tap the **×** on any thumb to drop a misshot before processing.
- One Gemini call costs **1 request** in your quota regardless of how many images. 10 books per request = 10× your effective daily limit.
...
- **Batch mode** (5 or 10 covers per request) divides the request-count cost across all images — same 250 RPD now covers 1250 or 2500 books.

### Why batch processing if quotas are so cheap on paid tier?
Mostly for staying free-tier-comfortable, and for handling intermittent network: one request to wait on instead of N. The catalogue logic still saves each book as its own row.
